...
15. **BEM modifier สำหรับ React Aria state** — ❌ ห้ามใช้ BEM class (`fri-input--disabled`, `fri-input--invalid`) สำหรับ state ที่ React Aria จัดการ. ใช้ `:has([data-X])` attr selector แทน. Component's bem() call ควรมีแค่ own modifiers (variant/size/radius). See `.claude/rules/styles.md` "React Aria First" section.
16. **Native pseudo-classes on React Aria components** — ❌ ห้ามใช้ `:hover` / `:focus` / `:active` ใน React Aria-wrapped components. ใช้ `[data-hovered]` / `[data-focused]` / `[data-focus-visible]` / `[data-pressed]` แทน. **Exception:** Radix-wrapped (Avatar) ใช้ native pseudos ได้เพราะ Radix ไม่ expose data attrs.
17. **Mutually exclusive focus states** — ❌ ห้ามใช้ `[data-focused]` ตัวเดียวสำหรับ mouse focus (match keyboard ด้วย). ต้องใช้ `:has([data-focused]):not(:has([data-focus-visible]))` สำหรับ mouse-only. `[data-focus-visible]` สำหรับ keyboard. See `.claude/rules/styles.md` "Mutually Exclusive Focus States".
18. **State priority: hover overrides focus** — ❌ `:has([data-hovered])` ที่มี `:not(readonly):not(disabled)` มี specificity สูงกว่า focus rule → hover wins. ต้องเพิ่ม `:not(:has([data-focused]))` ใน hover rule ให้ focus win. See `.claude/rules/styles.md` "State Priority" section.
