...
- **Over-declare in VariantsProps is banned**: `{name}.variants.ts` contains ONLY fried-ui-specific style/layout modifiers (`variant`, `size`, `radius`, `isFullWidth`, `isPending`, `isIconOnly`). Never redeclare props that the underlying primitive (React Aria / native HTML) already provides (`isDisabled`, `isInvalid`, `isReadOnly`, `isRequired`, `disabled`, `readOnly`, `required`, `aria-invalid`). They flow through via the intersection type `{Name}VariantsProps & Omit<ComponentPropsWithRef<Primitive>, "className" | "children">`. See `.claude/rules/architecture.md` "Over-declare".
- **Pass-through argTypes are required in stories**: Even though state props are NOT declared in VariantsProps, `meta.argTypes` + `meta.args` in `{name}.stories.tsx` MUST document them so Storybook controls work for CSS-effect toggling and LLMs reading `autodocs` on `npm publish` see the full accepted surface. React Aria wrappers use camelCase (`isDisabled`, `isInvalid`, `isReadOnly`, `isRequired`); thin native-HTML wrappers use native names (`disabled`, `readOnly`, `required`, `aria-invalid`). See `.claude/rules/storybook.md` "Pass-through Props".
- **Named imports with prefix (shadcn pattern)**: All subparts export as named imports with parent prefix — `FieldLabel`, `FieldDescription`, `FieldSet`, `FieldLegend`, `InputGroupAddon`, `InputGroupInput`, `AvatarImage`, `AvatarFallback`. NEVER use dot notation (`Field.Label`, `Avatar.Image`). Aligned with shadcn for LLM fluency + multi-framework port + tree-shake. See `.claude/rules/architecture.md` "Component Composition — Named Imports with Prefix".
- **Reuse existing primitives instead of creating redundant components**: If existing `Button`/`Input`/`Textarea` with variants/sizes covers the use case, use it directly. Example: `<Button size="sm" variant="ghost">` inside `<InputGroupAddon>` — don't create `InputGroupButton`. Only create new components when existing primitive can't be configured (e.g., `InputGroupInput` exists because Input inside group needs no border — a context that variant can't toggle cleanly).
- **LLM-first design**: fried-ui is consumed by AI builders (Claude plugins, v0, Figma Make, Cursor, Windsurf, Cline) as much as human developers. Every design decision must pass the LLM-first filter: semantic clarity, pattern uniqueness (no 2-ways), crawlable docs (llms.txt), AI tool integration (MCP server, Cursor rules, Claude plugin), shadcn-compatible naming where sensible. Multi-framework (React + WordPress + Laravel + plain HTML) is a hard constraint — all components must work as pure CSS classes without JS runtime.
